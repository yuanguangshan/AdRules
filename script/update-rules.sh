...
cat *.txt |grep '^/' |grep -v './\|.?\|.\$\|.js\|._\|.\*\|.(php|png)\|.[0-9]\|.\^\|.=\|.~' > l.txt
cat l.txt
...
cat .././mod/rules/*-rules.txt dns*.txt abp-hosts.txt *easylist*.txt l.txt | grep '^|\|^@\|^/' | grep -v './' | grep -v '\*' | grep -v '^\[' | grep -v '.\[' | grep -v '.\$'|grep -Ev "([0-9]{1,3}.){3}[0-9]{1,3}" | grep -v '^!' | sed 's/\^|/\^/' |sort -n | uniq | awk '!a[$0]++' > tmp-dns.txt  #处理DNS规则
