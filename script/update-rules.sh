...
cat .././mod/rules/adblock-rules.txt plus-easylist*.txt *easylist*.txt adguard*.txt | grep -v '^!' | grep -v '^！' | grep -v '^# ' | grep -v '^# ' | grep -v '^\[' | grep -v '^\【' | grep -v 'local.adguard.org' | sort -n | uniq | awk '!a[$0]++' >> tmp-adblock+adguard.txt #处理Plus规则
