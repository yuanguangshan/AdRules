...
do
  ...
  curl --parallel --parallel-immediate -k -L -C - -o "plus-easylist${i}.txt" --connect-timeout 60 -s "${easylist_plus[$i]}" | iconv -t GBK -c
...
cat .././mod/rules/adblock-rules.txt plus-easylist*.txt | sort -u | sort -n | uniq | awk '!a[$0]++' |grep -v '^!' | grep -v '^！' | grep -v '^# ' | grep -v '^# ' | grep -v '^\[' | grep -v '^\【' | grep -v 'local.adguard.org'  >> tmp-tmp-adblock+adguard.txt #处理Plus规则
...
set LC_ALL='C'
iconv -c -t utf-8 tmp-tmp-adblock+adguard.txt > tmp1-tmp-adblock+adguard.txt
